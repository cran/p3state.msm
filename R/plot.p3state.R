`plot.p3state` <-function(x,plot.trans=NULL,plot.marginal=NULL,plot.bivariate=NULL,time1,time2,xlab,ylab,zlab,col,...){
	...
	if (ntrans==3) {plot.marginal<-FALSE
...
if (plot.trans=="P11" | plot.trans=="all") {
		...
		for (k in 1:length(y)) {mydata2[k,2]<-p11LIDA(mydata,time1,mydata2[k,1])}
...
on.exit(devAskNewPage(oask))}
	...
	if (plot.trans=="P12" | plot.trans=="all") {
		...
		y1<-c(mydata[x,1]+0.000000001,mydata[x,4]+0.000000001)
		y2<-unique(y1)
		y<-sort(y2)
		y<-c(time1,y)
		mydata2<-matrix(data=0,ncol=2,nrow=length(y))
		for (k in 1:length(y)) {mydata2[k,1]<-y[k]}
		for (k in 1:length(y)) {mydata2[k,2]<-p12LIDA(mydata,time1,mydata2[k,1])}
...
on.exit(devAskNewPage(oask))}
	...
	if (plot.trans=="P22" | plot.trans=="P23" | plot.trans=="all") {
		...
		mydata2<-matrix(data=0,ncol=2,nrow=length(y))
		for (k in 1:length(y)) {mydata2[k,1]<-y[k]}
		for (k in 1:length(y)) {mydata2[k,2]<-p22LIDA(mydata,time1,mydata2[k,1])}
		if (ylab.aux==TRUE) ylab<-paste("Estimated prob. p22(",time1,",t)")
		if (plot.trans=="P22" | plot.trans=="all") matplot(mydata2[,1],mydata2[,2],xlab=xlab,ylab=ylab,type="s",...)
oask <- devAskNewPage(TRUE)
on.exit(devAskNewPage(oask))
		if (ylab.aux==TRUE) ylab<-paste("Estimated prob. p23(",time1,",t)")
		if (plot.trans=="P23" | plot.trans=="all") matplot(mydata2[,1],1-mydata2[,2],xlab=xlab,ylab=ylab,type="s",...)
oask <- devAskNewPage(TRUE)
on.exit(devAskNewPage(oask))}
	...
	if (plot.marginal=="TRUE") {
		...
		mydata2<-matrix(data=0,ncol=2,nrow=length(y))
		for (k in 1:length(y)) {mydata2[k,1]<-y[k]}
		for (k in 1:length(y)) {mydata2[k,2]<-Biv(mydata,max(mydata[,1]),mydata2[k,1])}
oask <- devAskNewPage(TRUE)
on.exit(devAskNewPage(oask))
...
on.exit(devAskNewPage(oask))}
	...
	if (plot.bivariate=="TRUE") {
		...
		persp(x1,y1,z,theta = 30,phi = 30,ticktype = "detailed", expand = 0.5,xlab=xlab, ylab=ylab, zlab=zlab,col=col,shade = 0.2,...)}
	
	if (ntrans==3 & pmar==TRUE) cat("The plot for the marginal distribution of the second time cannot be given for the illness-death model", "\n")
	if (ntrans==3 & pbiv==TRUE) cat("The plot for the bivariate distribution function cannot be given for the illness-death model", "\n")
}
